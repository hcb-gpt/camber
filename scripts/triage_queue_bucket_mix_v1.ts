...
const maxAgeDays = Math.min(
  Math.max(Number(flags["max-age-days"]) || 21, 1),
  365,
);
...
const ANON_KEY = String(
  Deno.env.get("CANONICAL_SUPABASE_ANON_KEY") ??
...
if (!ANON_KEY && !EDGE_SECRET) {
  throw new Error(
    "Missing auth: set CANONICAL_SUPABASE_ANON_KEY (preferred; fallback SUPABASE_ANON_KEY) or EDGE_SHARED_SECRET.",
  );
}
...
function isNeedsSplit(item: QueueItem): boolean {
  ...
  const taxonomy = String((item.context_payload || {})["taxonomy_state"] || "")
    .trim().toUpperCase();
...
function decideBucket(
  ...
  if (
    hasGuess && confidence !== null && confidence >= fastConfirmThreshold &&
    !hasWeakAnchor(item)
  ) {
    return "FAST_CONFIRM";
  }
...
const queueUrl =
  `${SUPABASE_URL}/functions/v1/bootstrap-review?action=queue&limit=${limit}&max_age_days=${maxAgeDays}`;
...
if (!skipTruthGraph) {
  ...
  const workers = Array.from({
    length: Math.min(concurrency, interactionIds.length),
  }, async () => {
    while (true) {
      const idx = i++;
      ...
      const tgUrl =
        `${SUPABASE_URL}/functions/v1/redline-thread?action=truth_graph&interaction_id=${
      ...
      try {
        ...
        truthByInteraction.set(interactionId, {
          ok: false,
        ...
        if (!flags.json) {
          console.warn(
            `[warn] truth_graph fetch failed for ${interactionId}: ${
...
if (total > 0) {
  if (
    !skipTruthGraph && truthGraphInteractionCount > 0 &&
    truthGraphFetchFailedRate > 0.05
  ) {
    decision =
    ...
    decision =
      "Prioritize pipeline repair + truth-graph defect UX before attribution UX polish.";
  } else if (needsSplitRate > 0.05) {
    decision =
      "Prioritize NEEDS_SPLIT forcing path (v1 hard-block, v2 interactive split).";
  } else if (pickRequiredRate > 0.5) {
    decision =
      "Prioritize picker-first anti-anchoring UX (no preselect, no confirm swipe under weak anchors).";
  } else if (bucketCounts.FAST_CONFIRM > 0) {
    decision =
      "Ship FAST_CONFIRM 1-gesture flow + QA sampling; keep receipts-only, no reasoning.";
  } else {
    decision =
      "Mixed queue; prioritize the largest bucket and keep truth-graph gating in place.";
  }
}
...
const summary = {
  ...
  queue: {
    ...
    truth_graph: skipTruthGraph ? null : {
      lane_counts: Object.fromEntries(
        [...truthLaneCounts.entries()].sort((a, b) => b[1] - a[1]),
      ),
      ...
      fetch_failed_rate: truthGraphInteractionCount > 0
        ? truthGraphFetchFailedRate
        : null,
    },
    confidence: {
      count: confs.length,
...
if (flags.json) {
  ...
  if (summary.queue.truth_graph) {
    ...
    for (
      const [lane, count] of Object.entries(
        summary.queue.truth_graph.lane_counts,
      )
    ) {
    ...
    if (summary.queue.truth_graph.fetch_failed_interactions > 0) {
      console.log(
        `  truth_graph_fetch_failed_interactions: ${summary.queue.truth_graph.fetch_failed_interactions} (${
          Math.round(
            ((summary.queue.truth_graph.fetch_failed_rate || 0) * 100) * 10,
          ) / 10
        }%)`,
      );
    }
  ...
  for (
    const [code, count] of Object.entries(summary.queue.reason_code_counts)
  ) {
