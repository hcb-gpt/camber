...
COMMENT ON VIEW v_call_ledger IS
  'Flat, one-row-per-call view joining calls_raw → spans → attributions → projects → contacts. Start here for call-level queries. Excludes shadow rows by default. project_name and project_id come from span_attributions (v4 SSOT), not interactions.project_id (deprecated).';
...
COMMENT ON VIEW v_interaction_primary_project IS
  'Per-interaction primary project from span_attributions. Deprecated columns interaction_project_id and primary_project_source removed 2026-02-19.';;
