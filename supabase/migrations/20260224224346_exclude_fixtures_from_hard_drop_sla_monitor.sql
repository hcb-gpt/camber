...
-- Migration: Exclude fixture/test interaction_ids from hard-drop SLA monitor
-- Fixes: get_hard_drop_sla_monitor() false positives on test data
-- Fixture prefixes excluded: cll_SHADOW%, cll_RACECHK%, cll_DEV%, cll_CHAIN%
-- Rollback: Re-create function without the WHERE exclusions in active_spans CTE
...
with active_spans as (
  ...
  where coalesce(cs.is_superseded, false) = false
    -- Exclude fixture/test interaction_ids
