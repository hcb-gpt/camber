-- P1 FIX: Filter shadow/test interactions from redline_contacts aggregates.
-- 353 polluting interactions (345 shadow + 8 unflagged test) inflate call_count,
-- claim_count, and last_snippet for 25+ real contacts.
-- Top impact: Randy Booth +31%, John Singleton +350%, Shayelyn Woodbery +73%.
--
-- Also backfills is_shadow=true on 31 unflagged test interaction rows so that
-- future queries only need the single `is_shadow IS NOT TRUE` predicate.

begin;

-- 1) Backfill: flag the 31 unflagged test interactions as shadow
update interactions set is_shadow = true
where is_shadow is not true
  and interaction_id ~* '(TEST|BYPASS|SMOKE|SYNTHETIC|ITEST|SUMMTEST|^canary_|^debug_|^zapier_test$|^test_ping$)';

-- 2) Recreate redline_contacts view with shadow filter on 3 CTEs
create or replace view redline_contacts as
with contact_lookup as (
  select distinct on (phone10)
    c.id, c.name, c.phone, phone10
  from (
...
sms_norm as (
  select sms_messages.id, sms_messages.sent_at, sms_messages.content,
    sms_messages.direction, sms_messages.contact_name, sms_messages.contact_phone,
...
owner_phone_digits as (
  ...
  from owner_phones where owner_phones.active = true
),
call_stats as (
  ...
  where interactions.contact_id is not null
    and interactions.is_shadow is not true
  group by interactions.contact_id
),
sms_stats as (
  select cl.id as contact_id, count(*) as sms_count, max(s.sent_at) as last_sms_at
  from sms_norm s
  join contact_lookup cl on cl.phone10 = s.phone10
  where s.phone10 <> ''::text
    and exists (select 1 from sms_norm s2 where s2.phone10 = s.phone10 and s2.direction = 'inbound'::text)
    and not (s.phone10 in (select owner_phone_digits.phone10 from owner_phone_digits where owner_phone_digits.phone10 <> ''::text))
  group by cl.id
),
grading_cutoff as (
  select coalesce(
    (select redline_settings.value_timestamptz from redline_settings where redline_settings.key = 'grading_cutoff'::text),
  ...
  select i.contact_id,
    ...
    count(distinct jc.id) filter (
      where jc.created_at >= (select grading_cutoff.cutoff from grading_cutoff)
        and not exists (select 1 from claim_grades cg2 where cg2.claim_id = jc.id and cg2.graded_at >= (select grading_cutoff.cutoff from grading_cutoff))
    ) as ungraded_count
  from interactions i
  join journal_claims jc on jc.call_id = i.interaction_id
  where i.contact_id is not null
    and i.is_shadow is not true
  group by i.contact_id
),
pending_review as (
  select coalesce(i.contact_id, c_match.id) as contact_id, rq.id as queue_id
...
review_stats as (
  select pending_review.contact_id, count(distinct pending_review.queue_id) as ungraded_count
  from pending_review where pending_review.contact_id is not null
  group by pending_review.contact_id
),
last_call as (
  select distinct on (i.contact_id)
    i.contact_id, left(i.human_summary, 80) as snippet, cr.direction,
    'call'::text as interaction_type, i.event_at_utc
  from interactions i
...
last_sms as (
  select distinct on (cl.id)
    cl.id as contact_id, left(s.content, 80) as snippet, s.direction,
    'sms'::text as interaction_type, s.sent_at as event_at_utc
  from sms_norm s
  join contact_lookup cl on cl.phone10 = s.phone10
  where s.phone10 <> ''::text
    and exists (select 1 from sms_norm s2 where s2.phone10 = s.phone10 and s2.direction = 'inbound'::text)
...
latest as (
  select distinct on (combined.contact_id)
    combined.contact_id, combined.snippet as last_snippet,
    combined.direction as last_direction, combined.interaction_type as last_interaction_type
  from (
...
select
  ...
  lt.last_snippet, lt.last_direction, lt.last_interaction_type
from contacts c
...
-- 3) Refresh both matviews to pick up the corrected counts
