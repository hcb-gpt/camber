-- P1: Enrich parity monitor payload with sample interaction IDs + invariants doc pointer.
-- Acceptance target: dispatch__p1_add_sample_ids_to_parity_alert_payload__data1__20260228
...
select
  ...
  coalesce(
    jsonb_agg(to_jsonb(r.camber_interaction_id) order by r.occurred_at_utc desc)
      filter (where r.rn <= 5 and coalesce(r.camber_interaction_id, '') <> ''),
    '[]'::jsonb
  ) as sample_interaction_ids
from ranked r;

comment on view public.v_beside_calls_missing_in_interactions_24h is
'24h parity summary: Beside call events that have no matching interactions(call/phone) row within ±120 seconds on normalized phone. Includes top-5 sample_interaction_ids from Beside payload.';
...
declare
  ...
  v_beside_sample_interaction_ids jsonb := '[]'::jsonb;
  v_interactions_sample_interaction_ids jsonb := '[]'::jsonb;
  v_invariants_doc_path text := 'docs/redline_pipeline_harness_invariants_v1_20260228.md';
...
begin
  select
    ...
    coalesce(v.sample_interaction_ids, '[]'::jsonb)
  into v_beside_missing_count, v_beside_examples, v_beside_sample_interaction_ids
  ...
  select
    coalesce(
      jsonb_agg(to_jsonb(s.interaction_id) order by s.event_at_utc desc),
      '[]'::jsonb
    )
  into v_interactions_sample_interaction_ids
  from (
    select distinct
      v.interaction_id,
      v.event_at_utc
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'sample_interaction_ids_beside_missing_in_interactions_24h', v_beside_sample_interaction_ids,
      'sample_interaction_ids_interactions_missing_in_redline_thread_24h', v_interactions_sample_interaction_ids,
      'example_beside_missing_tuples', v_beside_examples,
      'example_interactions_missing_rows', v_interactions_examples,
      'invariants_doc_path', v_invariants_doc_path
    ),
    not v_is_alert
  )
  ...
  if v_is_alert
    ...
    begin
      ...
      v_content := concat_ws(
        ...
        format('sample_interaction_ids_beside_missing=%s', v_beside_sample_interaction_ids::text),
        format('sample_interaction_ids_interactions_missing_in_redline=%s', v_interactions_sample_interaction_ids::text),
        format('invariants_doc_path=%s', v_invariants_doc_path)
      );
...
'Pipeline harness parity monitor: logs Beside->interactions and interactions->redline parity gaps, emits TRAM on threshold breach. Includes sample interaction IDs + invariants doc path.';
