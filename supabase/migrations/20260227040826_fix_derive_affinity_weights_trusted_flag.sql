-- Fix derive_affinity_weights to set camber.trusted_cpa_write flag
-- before UPDATE, allowing it to write weights for internal/floater contacts.
-- Without this flag, the trg_block_affinity_updates_internal_floater trigger
-- silently drops positive-weight updates for internal contacts.

CREATE OR REPLACE FUNCTION public.derive_affinity_weights(
  p_contact_id uuid DEFAULT NULL,
  p_human_boost numeric DEFAULT 10,
  p_rejection_penalty numeric DEFAULT 2,
...
BEGIN
  ...
  IF p_apply THEN
    -- Set trusted flag so trg_block_affinity_updates_internal_floater allows writes
