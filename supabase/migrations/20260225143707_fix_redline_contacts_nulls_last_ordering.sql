-- Fix: NULLS LAST on all DISTINCT ON ORDER BY clauses
-- Without this, NULL timestamps sort first in DESC and DISTINCT ON picks them
...
latest AS (
  ...
  ORDER BY contact_id, event_at_utc DESC NULLS LAST
)
SELECT
  c.id AS contact_id,
