-- Restore is_shadow filter on redline_contacts view
--
-- Migration 20260228030000_single_project_auto_attribution.sql used
-- DROP VIEW IF EXISTS public.redline_contacts CASCADE, which destroyed the
-- shadow/test exclusion filter added in 20260228015000. This caused 302+
-- shadow/test interactions to leak into the view, inflating call_count for
-- real contacts by up to 44%.
--
-- Fix: LEFT JOIN calls_raw on each interactions subquery and exclude rows
-- where cr.is_shadow IS TRUE. Applied to:
--   1. raw_stats → cs (call_count subquery)
--   2. raw_stats → cls (claim_stats subquery)
--   3. last_call CTE
--
-- All existing columns preserved:
--   contact_id, contact_name, contact_phone, company, role, contact_type,
--   is_ambiguous, call_count, sms_count, claim_count, ungraded_count,
--   last_activity, last_snippet, last_direction, last_interaction_type,
--   colliding_contact_ids
