-- Fix promote_daily_mismatches_to_manifest() CTE scoping bug (candidates count computed outside WITH)
...
begin
  ...
  select
    (select n from cand_ct),
    (select count(*)::int from updated)
  into v_candidates_count, v_promoted_count;

  v_skipped_count := greatest(v_candidates_count - v_promoted_count, 0);
