...
Deno.serve(async (req: Request) => {
  ...
  if (userError || !userData?.user) {
    return json({ ok: false, error: "unauthorized", detail: userError?.message }, 401);
  }
  ...
  if (req.method === "GET") {
    ...
    if (spanErr) {
      return json({ ok: false, error: "span_query_failed", detail: spanErr.message }, 500);
    }

    // Fetch existing feedback for these spans
    const spanIds = (spans ?? []).map((s: Record<string, unknown>) => s.span_id).filter(Boolean);
    ...
    if (spanIds.length > 0) {
      ...
      if (fb) {
        for (const row of fb as Array<{ span_id: string; verdict: string }>) {
  ...
  if (req.method === "POST") {
    ...
    if (!rawVerdict || !["CORRECT", "INCORRECT", "UNSURE"].includes(rawVerdict)) {
      return json({
        ok: false,
    ...
    const { error: insertErr } = await db.from("attribution_validation_feedback").insert({
      span_id: spanId,
      verdict: rawVerdict,
      notes: notes,
