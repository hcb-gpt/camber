export type EdgeSecretErrorCode = "invalid_edge_secret" | "edge_secret_missing" | "edge_secret_drift";
...
function readEdgeContractEnv(env: Record<string, string | undefined>) {
  const currentSecret = String(env.EDGE_SHARED_SECRET || "").trim();
  const nextSecret = String(env.EDGE_SHARED_SECRET_NEXT || "").trim();
  const nextExpiresAtUtc = String(env.EDGE_SHARED_SECRET_NEXT_EXPIRES_AT_UTC || "").trim();
...
export function authorizeEdgeSecretRequest(
  ...
  const providedSecret = String(req.headers.get("X-Edge-Secret") || req.headers.get("x-edge-secret") || "").trim();
  ...
  if (!values.currentSecret) {
    return {
      ...
      error: "EDGE_SHARED_SECRET is not configured",
      contract,
      current_secret: "",
    };
  }
