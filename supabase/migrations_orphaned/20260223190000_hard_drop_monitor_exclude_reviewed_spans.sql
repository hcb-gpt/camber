-- Hard-drop SLA monitor v1.1: exclude spans with llm_proxy_review feedback
--
-- Problem: review-swarm-runner writes to attribution_validation_feedback but
-- the monitor has no visibility into that table. Spans with feedback still
-- count as "pending" in the SLA breach calculation.
--
-- Fix: LEFT JOIN attribution_validation_feedback (source='llm_proxy_review')
-- in the pending_spans CTE. Spans with feedback are excluded from counts.
-- No destructive changes. No auto-resolve. No project assignment.
...
-- v1.1: Track spans that have been reviewed by llm_proxy_review
...
pending_spans as (
  ...
  left join reviewed_by_proxy rbp
    on rbp.span_id = s.span_id
  where
    (
      la.span_id is null
    ...
    -- v1.1: exclude spans that have been reviewed by llm_proxy_review
