-- Bridge trigger v4: Two new fallback resolution paths when contact_phone is NULL.
--
-- Guard 4 (NEW): beside_contact_id lookup
--   If sms_messages.beside_contact_id is populated, find any prior message
--   with the same beside_contact_id that has a resolved contact_phone.
--   Use that phone to resolve the contact.
--
-- Guard 5 (NEW): Temporal proximity
--   If still unresolved and direction='outbound', find the most recent
--   INBOUND SMS from a known contact within 30 minutes before this message.
--   Outbound replies typically follow inbound within seconds/minutes.
--
-- Bumps zap_version from sms_bridge_v3 to sms_bridge_v4.
--
...
BEGIN
  ...
  IF v_contact_id IS NULL THEN
    ...
    -- If this message has a beside_contact_id, find any prior message with the
    -- same beside_contact_id that has a resolved contact_phone. Use that phone.
  ...
  IF v_contact_id IS NULL THEN
    ...
    -- If this is an outbound message with no resolved contact, find the most
    -- recent inbound SMS from a known (resolved) contact within 30 minutes.
    -- Outbound replies typically follow inbound within seconds.
