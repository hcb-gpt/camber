...
--
-- Problem: Zack Sittler has 38 outbound-only SMS (texts FROM Chad TO Zack).
-- These inflate his sms_count and pollute his thread with one-sided messages
-- that have no inbound replies. The fix adds an EXISTS filter to both views
-- so that contacts with ZERO inbound SMS are excluded from SMS aggregation
-- entirely. Contacts with at least one inbound SMS keep all their messages
-- (both directions).

-- 1. redline_sms_thread — add WHERE EXISTS to exclude outbound-only contacts
...
-- 2. redline_contacts — add WHERE EXISTS to sms_stats and last_sms CTEs
