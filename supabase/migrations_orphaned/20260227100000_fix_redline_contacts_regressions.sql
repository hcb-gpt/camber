-- Fix two regressions introduced by 20260227075000 (phone normalization):
--
-- 1. pending_review CTE: re-added grading_cutoff filter (line 119) that
--    20260227052000 explicitly removed. Attribution items always need human
--    attention regardless of clock resets.
--
-- 2. ungraded_count column: dropped cls.ungraded_count fallback. Should be
--    COALESCE(rs.ungraded_count, cls.ungraded_count, 0) so contacts with
--    ungraded claims but no pending reviews still show their count.
--
...
pending_review as (
  -- FIX 1: NO grading_cutoff filter on attribution items.
  -- Attribution items always need human attention regardless of clock resets.
  -- (Regression from 075000 that re-added filter removed by 052000.)
  select
...
select
  ...
  -- FIX 2: Restore cls.ungraded_count fallback for contacts with ungraded
  -- claims but no pending review items.
